...
class Ion_auth_model extends CI_Model
{
	...
	*/
	public function register($identity, $password, $email, $additional_data = array())
	{
		...
		return $id = $this->db->insert_id($this->tables['users']);
    // Lets create addtional profile
		// $this->create_profile($additional_data, $id);
		// if ($additional_data['wakalah_id']) {
		// 	$this->assign_wakalah_user($additional_data, $id);
		// }
		...
		//   if($this->input->post('type')=='admin_kgt'){
		//     $groupData=array('105','109','110');
		//   }
		//   if($this->input->post('type')=='staff_kgt'){
		//     $groupData=array('105','106','107');
		//   }
		// }
		// if($this->input->post('user_group')=='2'){
		//   if($this->input->post('type')=='admin_wak'){
		//     $groupData=array('103');
		//   }
		//   if($this->input->post('type')=='staff_wak'){
		//     $groupData=array('104');
		//   }
		// }
		...
		// 			$this->db->insert($this->tables['users_groups'],
		// 			array(
		// 				$this->join['groups'] => $group,
		// 				$this->join['users']  => $id
		// 			));
		//
		// 	}
		//}
		//$this->db->insert('users_groups',array('user_id'=>$id,'group_id'=>$this->input->post('type')));

		//$this->trigger_events('post_register');

		// Custom by shamie
		//$p=json_encode($groupData);
		// $m="Daftar Pengguna Berjaya";
	...
	*
	*/
	public function create_profile($additional_data, $id)
	{
		$profile_data = array(
			'user_id' => $id,
			'full_name' => strtoupper($additional_data['full_name']),
			'phone' => $additional_data['phone'],
			'address' => strtoupper($additional_data['address']),
			'postcode' => $this->input->post('postcode'),
			'town_area' => strtoupper($this->input->post('town_area')),
			'state_id' => $this->input->post('state_id'),
			//'nic_no' => strtoupper($additional_data['nic_no']),
		);

		$this->db->insert($this->tables['users_profile'], $profile_data);

		if($this->input->post('user_group')==2){
			// $bill_data = array(
			// 	'user_id' => $id,
			// 	'bill_name' => strtoupper($additional_data['full_name']),
			// 	'bill_phone' => $additional_data['phone'],
			// 	'bill_address' => strtoupper($additional_data['address']),
			// 	'bill_email' => $this->input->post('email'),
			// 	'bill_default' => 1,
			// );
			// $this->db->insert('ci_billing', $bill_data);

			$ship_data = array(
				'user_id' => $id,
				'ship_name' => strtoupper($additional_data['full_name']),
				'ship_phone' => $additional_data['phone'],
				'ship_address' => strtoupper($additional_data['address']),
	...
	*
	*/
	public function assign_wakalah_user($additional_data, $id)
	{
		$user_wakalah = array(
			'user_id' => $id,
			'wakalah_id' => $additional_data['wakalah_id']
		);
		$this->db->insert('ci_user_wakalah', $user_wakalah);
	}
	...
	*/
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ',username, email, id, password, active, last_login, user_group, verify_acc')
		->where($this->identity_column, $identity)
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				if ($user->active == 0 || $user->user_group==2 || $user->verify_acc == 0)
				{
					$this->trigger_events('post_login_unsuccessful');
	...
	public function login_cust($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
	...
	public function login_seller($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		->where($this->identity_column, $identity)
		->or_where('email',$identity)
		->join('ci_seller', 'ci_seller.user_id = ' . $this->tables['users'] . '.id', 'inner')
		->limit(1)
		->order_by('id', 'desc')
		->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				if ($user->active == 0 || $user->user_group!=2 || $user->verify_acc == 0 || $user->seller_status == 0)
				{
					$this->trigger_events('post_login_unsuccessful');
					$this->set_error('login_unsuccessful_not_active');
	...
	*/
	public function user($id = NULL)
	{
		...
		$this->db->join('ci_users_profile', 'ci_users_profile.user_id ='.$this->tables['users'].'.id');
	...
	*/
	public function add_to_group($group_ids, $user_id = FALSE)
	{
		...
		* @author Ben Edmunds
		*/
		public function groups($group_role = '')
		{
			...
			$this->db->where($this->tables['groups'].'.group_role', $group_role);
